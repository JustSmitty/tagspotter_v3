 ...
 *   node scripts/make-screenshot-seed.mjs --js f     # write the injectable script
...
if (jsIndex !== -1 && args[jsIndex + 1]) {
  // The script the screenshot workflow injects into the built web assets. It
  // defines the global PreferenceStorageService reads, keyed exactly as the app
  // keys its Preferences — see StateService's unifiedStorageKey and
  // onboardingSeenKey. Values are strings because that is what Preferences
  // returns; the save is a JSON string, and the onboarding flag is the literal
  // 'true' the standalone reader compares against.
  //
  // Same-origin classic script, so `script-src 'self'` covers it (con-0002).
  const seedMap = {
    tagspotter_v1_save_data: json,
    tagspotter_v1_onboarding_seen: 'true',
  };
  const js = `// Generated by scripts/make-screenshot-seed.mjs — screenshot builds only.
window.__TAGSPOTTER_SCREENSHOT_SEED__ = ${JSON.stringify(seedMap)};
`;
  writeFileSync(args[jsIndex + 1], js, 'utf8');
  console.log(`wrote ${js.length} bytes of seed script to ${args[jsIndex + 1]}`);
  process.exit(0);
}
